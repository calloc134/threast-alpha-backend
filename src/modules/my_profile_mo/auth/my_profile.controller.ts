...
import { Controller, Get, Post, Session, Body, Put, Delete, ParseIntPipe, Query } from '@nestjs/common';
...
export class AuthMyProfileController {
  ...
  @Put('me/password')
  @ApiOkResponse({ type: StandardUserResDto })
  async updateMyPassword(@Session() session: SecureSession, @Body() updateUserPasswordReqDto: UpdateUserPasswordReqDto) {
    return await this.authMyProfileService.updateMyPassword(session.user_cuid, updateUserPasswordReqDto);
  }

  @Get('me/posts')
  @ApiOkResponse({ type: PaginatedResDtoTinyPost })
  async getMyPosts(@Query() query: PaginatedQueryReqDto, @Session() session: SecureSession) {
    return await this.authMyProfileService.getMyPosts(session.user_cuid, query.current_page ?? 1, query.per_page ?? 10);
  }

  @Get('me/followings')
  @ApiOkResponse({ type: PaginatedResDtoTinyUser })
  async getMyFollowings(@Query() query: PaginatedQueryReqDto, @Session() session: SecureSession) {
    return await this.authMyProfileService.getMyFollowings(session.user_cuid, query.current_page ?? 1, query.per_page ?? 10);
  }

  @Get('me/followers')
  @ApiOkResponse({ type: PaginatedResDtoTinyUser })
  async getMyFollowers(@Query() query: PaginatedQueryReqDto, @Session() session: SecureSession) {
    return await this.authMyProfileService.getMyFollowers(session.user_cuid, query.current_page ?? 1, query.per_page ?? 10);
  }

  @Post('me/followings/:handle')
  @ApiOkResponse({ type: TinyUserResDto })
  async createMyFollowing(@Session() session: SecureSession, @Query('handle') dst_user_handle: string) {
    return await this.authMyProfileService.createMyFollowing(session.user_cuid, dst_user_handle);
  }
}
